...
if "generating_playlist" not in st.session_state:
    st.session_state.generating_playlist = (
        False  # Track if playlist generation is in progress
    )
if "cancel_generation" not in st.session_state:
    st.session_state.cancel_generation = (
        False  # Track if user wants to cancel generation
    )
if "pending_playlist" not in st.session_state:
    st.session_state.pending_playlist = (
        None  # Track if there's a playlist awaiting approval
    )
...
def get_user_top_tracks(limit=50):
    ...
    if st.session_state.spotify_client:
        return st.session_state.spotify_client.get_user_top_tracks(limit=limit)
    return None
        ...
        if st.button("✅ Create Playlist", type="primary", use_container_width=True):
            ...
            with st.spinner("📝 Creating playlist on Spotify..."):
                ...
                if playlist_id:
                    ...
                    st.session_state.spotify_client.add_tracks_to_playlist(
...
def generate_playlist_interface():
    ...
    if st.button(
        ...
        type="primary",
        use_container_width=True,
        disabled=st.session_state.generating_playlist,
    ):
    ...
    if st.session_state.generating_playlist:
        ...
        with col2:
            if st.button(
                "❌ Cancel Generation", type="secondary", use_container_width=True
            ):
                st.session_state.generating_playlist = False
        ...
        with st.spinner("🤖 Analyzing your taste and generating playlist..."):
            ...
            if not st.session_state.cancel_generation:
                ...
                try:
                    ...
                    used_songs = (
                        set()
                    )  # Track songs we've already tried to avoid duplicates
                    ...
                    while len(track_uris) < playlist_length and attempt <= max_attempts:
                        ...
                        if attempt == 1:
                            ...
                            st.write(
                                f"🔄 Need {needed} more songs, trying again (attempt {attempt})..."
                            )
                        ...
                        if not recommendations:
                            if attempt == 1:
                                st.error(
                                    "AI could not generate recommendations. Please try a different prompt."
                                )
                                st.session_state.generating_playlist = False
                                return
                            else:
                        ...
                        for i, recommendation in enumerate(recommendations):
                            ...
                            try:
                                ...
                                if " by " in recommendation:
                                    song_name, artist_name = recommendation.split(
                                        " by ", 1
                                    )
                                ...
                                progress_bar.progress(
                                    min(base_progress + 5 + search_progress, 90)
                                )
                        ...
                        if (
                            len(track_uris) < playlist_length
                    ...
                    if not track_uris:
                        st.error(
                            "Could not find any of the recommended songs on Spotify."
                        )
                        st.session_state.generating_playlist = False
                        return
...
def main():
    ...
    st.markdown(
        '<p style="text-align: center; font-size: 1.2rem; color: #666;">AI-Powered Playlist Generation Tool</p>',
        unsafe_allow_html=True,
    )
    ...
    with st.sidebar:
        ...
        if not st.session_state.authenticated:
            ...
            if st.button(
                "🎵 Connect to Spotify", type="primary", use_container_width=True
            ):
            ...
            if st.session_state.user_profile:
                ...
                st.success(
                    f"🎵 {st.session_state.user_profile.get('display_name', 'User')}"
                )
